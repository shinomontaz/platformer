...
func NewPhys2(r pixel.Rect, opts ...PhysOption) Phys2 {
	p := Phys2{
		rect:           r,
...
func (p *Phys2) GetVel() *pixel.Vec {
	return &p.vel
}

func (p *Phys2) SetMass(m float64) {
	p.mass = m
}

func (p *Phys2) IsGround() bool {
	return p.isground
}

func (p *Phys2) Apply(force pixel.Vec) {
	p.force = p.force.Add(force)
}

func (p *Phys2) SetSpeed(newspeed pixel.Vec) {
	p.vel = newspeed
}

func (p *Phys2) Update(dt float64, objs []Objecter) {
	p.currObjs = objs
...
func (p *Phys2) StepPrediction(v pixel.Vec) (float64, pixel.Vec, pixel.Vec) {
	ground := 0.0
	vel := p.vel
...
func (p *Phys2) Move(v pixel.Vec) {
	p.rect = p.rect.Moved(v)
}

func (p *Phys2) GetRect() pixel.Rect {
	return p.rect
}

func (p *Phys2) Draw(t pixel.Target) {
	imd := imdraw.New(nil)
